<%@page import="com.dami.category.bean.category"%>
...
<body style="background-color: #fafafa;">
		...
		<div class="scroll_left" style="height: 420px;">
			<ul class="scr-ul">
				...
				<c:forEach items="${clist }" var="cl">
					...
					<li class="scr_li"><a href="">${cl.name }</a><i class="scr_i"></i></li>
				</c:forEach>
			</ul>
		</div>
	</div>
		...
		<div class="bot_max">
			<div class="bot_first">
				<div class="bot_one">
					...
					<div>
						<a href=""><img src="img/bot_04.jpg">img粉条卡</a>
					</div>
					<div>
	...
	<div class="time">
		<div class="H">大米明星单品</div>
		<div class="time_in">
			<!-- 动态数据时使用，已经修改过css -->
			<c:forEach items="${glist }" var="gl">
				<div
					style="background-color: #fff; width: 234px; height: 320px; float: left; margin-left: 11px;">
					<a href="detailServlet?method=detail&gid=${gl.gid }" target="_blank"> <img class="time_min"
						style="width: 234px; height: 234px;" src="<%=imgPath %>${gl.pic }"
						alt="">
					</a>
			...
			<!-- 原始代码 -仅供参考 -->
			<%--  <div> 
	            <a href=""><img  src="<%=imgPath %>${gl.pic }" alt="" class="" ></a>
	           </div>  --%>
		</div>
	</div>
			...
			<div class="app_max">
				...
				<div class="appr">
					<div class="appr_top">
						...
						<c:forEach items="${jiadian1 }" var="ji">
							<div class="appr_min" id="appr_min1"
								style="margin-left: 12px; width: 234px; height: 300px; background-color: #FFF; text-align: center;">

								<a href="detailServlet?method=detail&gid=${ji.gid }" target="_blank"><img width="180px"
									height="180px;" src="<%=imgPath %>${ji.pic }" alt=""></a>

								<div style="width: 234px; height: 100px;">
									<div
										style="width: 100%; height: 33px; text-align: center; font-size: 14px; font-family: Arial">${ji.gname }</div>
									<div
										style="width: 100%; height: 33px; text-align: center; font-size: 14px; font-family: Arial">
										<c:if test="${ji.description.length() <=12 }">
		            				${ji.description}</c:if>
		            				<c:if test="${ji.description.length() > 12 }">
		            				${ji.description.substring(0,12) }...
		            			</c:if>
										</div>
									<div
										style="width: 100%; height: 33px; text-align: center; color: #ff6700; font-size: 14px; font-family: Arial">${ji.price }</div>
								</div>

							</div>
						</c:forEach>
						<c:forEach items="${jiadian2 }" var="ji">
							<div class="appr_min"
								style="margin-left: 12px; width: 234px; height: 300px; background-color: #FFF; text-align: center; margin-top: 15px;"
								id="appr_min1">
								<a href="detailServlet?method=detail&gid=${ji.gid }"><img width="180px" height="180px;"
									src="<%=imgPath %>${ji.pic }" alt=""></a>
								<div style="width: 234px; height: 100px;">
									<div
										style="width: 100%; height: 33px; text-align: center; font-size: 14px; font-family: Arial">${ji.gname }</div>
									<div
										style="width: 100%; height: 33px; text-align: center; font-size: 14px; font-family: Arial"><c:if test="${ji.description.length() <=12 }">
		            				${ji.description}</c:if>
		            				<c:if test="${ji.description.length() > 12 }">
		            				${ji.description.substring(0,12) }...
		            			</c:if></div>
			...
			<div class="capa_box scrip_capa_box scrip_capa_box_on">
				<div class="capa_top">
					...
					<!-- 原始网页，仅供参考 -->
						...
						<c:forEach items="${zhineng }" var="zh">
					 <div class="capa capa_shadow" style="width: 231px;height: 301px;text-align: center;background-color: #FFFFFF;margin-top: 15px;">
	                        <a href="detailServlet?method=detail&gid=${zh.gid }" target="_blank"><img width="180px" height="180px" src="<%=imgPath%>${zh.pic }">
	                            <div class="tran">
	                       <span>${zh.full_description }</span>
	                            </div>
	                        </a>
	                        <div class="capa_a">   
	                            <ul>
	                                <li style="color: #333333">${zh.gname }</li>
	                                <li style="color: #b0b0b0">${zh.description }</li>
	                                <li style="font-size: 14px;margin-top: 10px; color: #ff6700" >${zh.price }元</li>
	                            </ul>
	                        </div>
	                    </div> 
				</c:forEach>
		...
		<div class="capacity">
			...
			<div class="capa_box">
				<div class="capa_top">
					...
					<!-- 动态数据使用，仅供参考 -->
					<c:forEach items="${tuijian }" var="comm">
	                	<div class="capa capa_shadow" style="margin-right: 10px;background-color: #fff">
	                        <a href="detailServlet?method=detail&gid=${comm.gid }"><img width="234"  src="<%=imgPath %>${comm.pic}">
	                           
	                        </a>
	                        <div class="capa_a">
	                            <ul>
	                                <li style="color: #333333"> ${comm.gname } </li>
	                                <li style="color: #b0b0b0"> ${comm.full_description } </li>
					...
					<%--<div class="capa capa_shadow">
					 ...
					 <div class="capa capa_shadow">
						<a href="indexServlet?fs=findGoodsDetail&gid=${goods.gid }"
							target="_blank"><img src="img/capa_02.jpg"> <!--<div class="tran">-->
							<!--<span>小米一如既往的好前天下单今天就到了，送货到家很细心的...</span>--> <!--</div>--> </a>
						<div class="capa_a">
							<ul>
								<li style="color: #333333">米家LED 智能台灯</li>
								<li style="color: #b0b0b0">无可视频闪,四种场景可变调光</li>
					...
					<div class="capa capa_shadow">
						<a href=""><img src="img/capa_03.jpg"> <!--<div class="tran">-->
							<!--<span>现在看起来挺不错的，性价比超高！持续关注呀</span>--> <!--</div>--> </a>
						<div class="capa_a">
							<ul>
								<li style="color: #333333">九号平衡车 Plus</li>
								<li style="color: #b0b0b0">35km超长续航,一键自动跟随</li>
					...
					<div class="capa capa_shadow capa_unright">
						<a href=""><img src="img/capa_05.jpg" alt=""> <!--<div class="tran">-->
							<!--<span>小米一如既往的好前天下单今天就到了，送货到家很细心的...</span>--> <!--</div>--> </a>
						<div class="capa_a">
							<ul>
								<li style="color: #333333">米家恒温电水壶</li>
	...
	<div class="popular">
		<div class="popular_background">
			...
			<div class="popu_box">
				...
				<c:forEach items="${hotGoods }" var="goods" varStatus="i">
					...
					<c:if test="${i.count ==1 }">
						<div class="popu popu_unleft popu_shadow">
							<a href="detailServlet?method=detail&gid=${goods.gid }"
								target="_blank"><img style="width: 296px" height="220px"
								src="<%=imgPath %>${goods.pic}"></a>
							<div class="popu_bottom">
								<p class="review">
									<c:if test="${goods.full_description.length()>25} ">${goods.full_description.substring(0,25) } </c:if>
									<c:if test="${goods.full_description.length()<25} ">${goods.full_description }</c:if>
								</p>
				...
				<script>
					var abc = document
							.getElementsByClassName("script_capa_box_top_ar");
					console.log("abc:" + abc);
					...
					function fun() {
						...
						$(".scroll>ul>li").css("opacity", "0").eq(n).css(
								"opacity", "1")
						$(".scroll_dot span").eq(n).addClass("scroll_dot_span")
								.siblings().removeClass("scroll_dot_span");
					}
